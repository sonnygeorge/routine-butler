...
#                     )
    ...
    def on_select_program(self, new_program: int):
        logger.debug(
            f"Routine item {self.item_id} program changed to {new_program}"
        )
        with self.ctl.session() as session:
...
class RoutineItemsExpansion(SidebarExpansion):
    def __init__(self, ctl: RoutineCtl, routine: Routine):
        self.ctl = ctl
        ...
        with self:
            ...
            self._update_items_frame(routine)
            ...
            with ui.row().classes(DFLT_ROW_CLASSES + f" pb-{V_SPACE}"):
                ...
                self.add_routine_item_button.on("click", self.on_add_routine_item)
#                 # add reward buttom
...
#                 )
    ...
    def on_add_routine_item(self):
        ...
        with self.ctl.session() as session:
            ...
            routine_item = RoutineItem(routine=routine, order_index=self.last_reg_idx + 1)
            ...
            routine.routine_items.append(routine_item)
            # update self.last_reg_idx
            self.last_reg_idx += 1
            logger.debug(f"Last regular routine item idx now: {self.last_reg_idx}")
            if any(ri.is_reward for ri in routine.routine_items):
...
#         session.commit()
    ...
    def _update_items_frame(self, routine):
...
class RoutineConfigurer(SidebarExpansion):
    def __init__(
        ...
        with self:
            ...
            with ui.row().classes(DFLT_ROW_CLASSES):
                RoutineItemsExpansion(self.ctl, routine)
...
#                 )
    ...
    def on_title_update(self, new_title):
        logger.debug(
            f'Updating title of routine {self.routine_id} to "{new_title}"'
        )
        with self.ctl.session() as session:
            self.header_label.set_text(new_title)
            self.ctl.rename_routine(session, self.routine_id, new_title)
...
def routine_sidebar(ctl: RoutineCtl, user: User):
    ...
    def on_add_routine():
        logger.debug("Adding routine for user: " + user.username)
        with routines_frame:
            with ctl.session() as session:
                routine = ctl.add_routine(session)
                RoutineConfigurer(ctl, routine, parent_element=routines_frame)
    ...
    with drawer:
        ...
        add_routine_button.props("icon=add").on("click", on_add_routine)
    return drawer
